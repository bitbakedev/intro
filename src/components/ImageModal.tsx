...
const ImageModal: React.FC<ImageModalProps> = ({ isOpen, onClose, imageSrc, imageAlt }) => {
  ...
  return (
    ...
    >
      {/* 컨트롤 버튼들 - PC에서만 표시 */}
      <div className="absolute top-4 right-4 flex space-x-2 z-10 hidden md:flex" onClick={(e) => e.stopPropagation()}>
        <button
      ...
      {/* 확대/축소 정보 - PC에서만 표시 */}
      <div className="absolute top-4 left-4 bg-white bg-opacity-20 px-3 py-1 rounded-full z-10 hidden md:block" onClick={(e) => e.stopPropagation()}>
      ...
      {/* 이미지 컨테이너 */}
      <div
        className="relative max-w-4xl max-h-[90vh] overflow-hidden flex items-center justify-center"
      ...
      {/* 사용법 안내 - PC에서만 표시 */}
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white bg-opacity-20 px-4 py-2 rounded-full hidden md:block" onClick={(e) => e.stopPropagation()}>
