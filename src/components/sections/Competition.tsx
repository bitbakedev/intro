...
const Competition: React.FC = () => {
  ...
  return (
    <>
      ...
      <p className="text-gray-700 leading-relaxed mb-4">
         비트베이크는 암호화폐를 직접 리워드로 지급함으로써 PiNetwork 와 같이 글로벌 확장이 가능합니다. 더 나아가 이를 스테이킹으로 연계해 앱 유저의 자산을 락인(lock-in) 시키는 구조를 갖추고 있어, 명확한 수익 구조 없이 토큰 상장에만 의존하는 기존 프로젝트(Pi Network 와 같은 Web3 프로젝트) 들과 달리, 지속 가능한 수익성과 유저 리텐션을 동시에 확보한 모델이라는 점에서 차별화됩니다.
      </p>
    </div>
